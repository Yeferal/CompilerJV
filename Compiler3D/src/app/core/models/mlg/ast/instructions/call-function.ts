...
export class CallFunction extends Node {
	...
	constructor(positionToken: PositionToken, toke: string, id: string, params: Array<Node>) {
    ...
    public override executeComprobationTypeNameAmbitUniqueness(handlerComprobation: HandlerComprobation): any {
        ...
        if (!this.isTypeCorrect(symbolFunc.symbolType)) {
            ...
            const errorGramm = new ErrorGramm(this.positionToken, this.toke, `No existe una funcion o procedimiento << ${this.id} >>.`, ErrorType.SEMANTIC); 
            handlerComprobation.listError.push(errorGramm);
            return this.type;
        }

        //Ejecutar las comprobaciones para obtener los tipos de datos de los parametros
        let listParamsOfNode = new Array<DataType>;
        ...
        if (this.params.length != symbolFunc.listParams.length) {
            ...
            const errorGramm = new ErrorGramm(this.positionToken, this.toke, `El numero de parametros no coincide en la funcion << ${this.toke} >>.`, ErrorType.SEMANTIC); 
            handlerComprobation.listError.push(errorGramm);
        } else {
            ...
            if (this.params.length > 0) {
                ...
                for (let i = 0; i < listParamsOfNode.length; i++) {
                    if (listParamsOfNode[i] != paramSymbol[i]) {
                        ...
                        const errorGramm = new ErrorGramm(this.positionToken, this.toke, `El tipo de dato de los parametros no coincide en la funcion << ${this.toke} >>.`, ErrorType.SEMANTIC); 
                        handlerComprobation.listError.push(errorGramm);
                    }
                }
            }
