...
const generateAssmTHead = (listTempsString) => {
    let text = "";
    text += "section .data \n"
    text += "\tstack resq 3000            ; Reservar espacio para 3000 elementos de 8 bytes en el stack (float)\n"
    text += "\theap resq 3000             ; Reservar espacio para 3000 elementos de 8 bytes en el heap (float)\n"
    text += "\tptr resd 1                  ; Reservar 4 bytes para ptr (int)\n"
    text += "\th resd 1                    ; Reservar 4 bytes para h (int)\n"
    text += "\tps resd 1                   ; Reservar 4 bytes para ps (int)\n"
    
    return text;
}

const generateAssmTInt = (listTempsInt) => {
    let text = "";
    for (let i = 0; i < listTempsInt.length; i++) {
        text += `\tt${listTempsInt[i]} resd 1                   ; Reservar 4 bytes para t${listTempsInt[i]} (int)\n`
        
    }
    return text;
}

const generateAssmTFloat = (listTempsFloat) => {
    let text = "";
    for (let i = 0; i < listTempsFloat.length; i++) {
        text += `\tt${listTempsFloat[i]} resq 1                   ; Reservar 8 bytes para t${listTempsFloat[i]} (float)\n`
        
    }
    return text;
}

const generateAssmTString = (listTempsString) => {
    let text = "";
...
const generateAssmQuartet = (listQuartet) => {
    let text = "";
